...
public class Servlet_Prod extends HttpServlet {
    ...
    private void RegistrarProducto(HttpServletRequest request, HttpServletResponse response)
        ...
        if(resp){
            response.sendRedirect("mensaje.jsp?mens='Se ha registrado el producto correctamente'"); 
        }else{
            response.sendRedirect("mensaje.jsp?mens='Error al registrar el producto'"); 
        }
    }
